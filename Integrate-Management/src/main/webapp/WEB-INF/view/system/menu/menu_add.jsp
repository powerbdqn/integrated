...
<body >
	<form id="menuForm" method="post" action="${pageContext.request.contextPath}/menu/menu_save.do" style="padding:30px;">
		<input type="hidden" name="id" value="${menu.id}" class="easyui-validatebox"  style="margin-bottom:10px" />
		<label>菜单名称</label>	
		...
		<label>父项菜单</label>
		<input id="selectComb" name="parentMenu.id" value="${menu.pid}" class="easyui-combobox" data-options="valueField:'id',textField:'title',url:'${pageContext.request.contextPath}/menu/parent_menu_list.do'" /><br/>
		<label>描述</label>	
